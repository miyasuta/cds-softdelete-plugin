...
| 照会時 | ドラフト | 子      | `$filter=isDeleted eq true` | `isDeleted=true` の子ドラフトのみ返す  | R2（明示的フィルタは機能する）         |
...
| 照会時 | ドラフト | 子  | 親未削除 / Navigation、`$filter=isDeleted eq true` | `isDeleted=true` の子のみ返す  | R2（明示的フィルタは機能する）            |
| 照会時 | ドラフト | 子  | 親未削除 / Navigation、`isDeleted` 指定なし or false   | `isDeleted=false` の子のみ返す | R2（明示的フィルタは機能する）            |
