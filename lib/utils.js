...
async function softDeleteCompositionChildren(entity, keys, deletionData) {
    ...
    for (const [elementName, element] of Object.entries(entity.elements)) {
        ...
        try {
            ...
            // Filter out records that are already soft-deleted
            // We should not update deletedAt/deletedBy for records that were previously deleted
            const notDeletedRecords = childRecords.filter(record => !record.isDeleted)
            const alreadyDeletedRecords = childRecords.filter(record => record.isDeleted)

            if (alreadyDeletedRecords.length > 0) {
                LOG.debug(`Skipping ${alreadyDeletedRecords.length} already deleted records in ${targetEntityName}`)
            }

            // Soft delete only records that are not already deleted
            if (notDeletedRecords.length > 0) {
                // Build WHERE condition for only non-deleted records
                // We need to target specific records by their keys
                for (const record of notDeletedRecords) {
                    const recordKeyConditions = []
                    for (const keyName of Object.keys(targetEntity.keys || {})) {
                        if (!draftVirtualKeys.includes(keyName)) {
                            recordKeyConditions.push({ ref: [keyName] }, '=', { val: record[keyName] })
                            recordKeyConditions.push('and')
                        }
                    }
                    // Remove trailing 'and'
                    if (recordKeyConditions.length > 0) {
                        recordKeyConditions.pop()
                        await UPDATE(targetEntityName)
                            .set(deletionData)
                            .where(recordKeyConditions)
                    }
                }
            }
